...
from tasks.jobs_task import create_process_job, add_file
...
def main():
    ...
    with app.app_context():
        ...
        try:
            for fid in file_ids:
                create_process_job(args.graph, fid, JobStage.KNOWLEDGE_TO_SAVE.value)
        except Exception as e:
            print(f"创建处理任务失败: {e}")
            traceback.print_exc()

        # start JobChecker which will poll DB and orchestrate tasks
        checker = JobChecker(graph_name=args.graph, app=app)
        try:
            checker.start()
