"""Generated-resource persistence and type dispatch.

This module owns the deterministic part of resource generation: validating
typed content, rendering files, writing manifests, and dispatching generated
payloads by resource type. Agent orchestration stays in
...
def _normalize_practice_language(value: Any) -> str:
    language = _safe_text(value).lower()
    return language or "python"


def _normalize_practice_steps(raw_steps: Any, *, question: str) -> list[dict]:
    steps = raw_steps if isinstance(raw_steps, list) else []
    normalized = []
    for index, step in enumerate(steps, start=1):
        if isinstance(step, dict):
            title = _safe_text(step.get("title") or step.get("name") or step.get("step"))
            instruction = _safe_text(step.get("instruction") or step.get("description") or step.get("content"))
            try:
            ...
            title = f"Step {index}"
            instruction = _safe_text(step)
            step_index = index
        if instruction:
            normalized.append({"step_index": step_index, "title": title or f"Step {index}", "instruction": instruction})
    return normalized or [
        {"step_index": 1, "title": "Read the goal", "instruction": f"Understand the question first: {question}"},
        {"step_index": 2, "title": "Run the example", "instruction": "Run the example code and inspect the output."},
    ]


def _default_practice_code_file(language: str, topic: str) -> dict:
    if language == "java":
        return {
            "path": "code/Main.java",
            "purpose": "entry",
            "content": (
                ...
                f"        System.out.println(\"resource for {topic}\");\n"
                "    }\n"
                "}\n"
            ),
        }
    return {
        "path": "code/main.py",
        "purpose": "entry",
        "content": f"def explain():\n    return 'resource for {topic}'\n\nprint(explain())\n",
    }


def _normalize_practice_code_files(raw_code_files: Any, *, language: str, topic: str) -> list[dict]:
    code_files = raw_code_files if isinstance(raw_code_files, list) else []
    extension = ".java" if language == "java" else ".py"
    normalized = []
    for index, item in enumerate(code_files, start=1):
        if isinstance(item, dict):
            content = _safe_text(item.get("content") or item.get("code") or item.get("source"))
            path = _safe_text(item.get("path") or item.get("file_path") or item.get("filename") or item.get("name"))
            purpose = _safe_text(item.get("purpose")) or ("entry" if index == 1 else "support")
        else:
            content = _safe_text(item)
            path = ""
            purpose = "entry" if index == 1 else "support"
        if not content:
            continue
        if not path:
            path = "code/Main.java" if language == "java" and index == 1 else f"code/file_{index}{extension}"
        elif "/" not in path and "\\" not in path:
            path = f"code/{path}"
        if "." not in path.rsplit("/", 1)[-1]:
            path = f"{path}{extension}"
        normalized.append({"path": path, "purpose": purpose, "content": content})
