...
def test_build_learning_profile_uses_behavior_answer_and_resource_signals(monkeypatch, repo_json_factory):
	...
	user_syllabus = SimpleNamespace(syllabus_id=19, personal_syllabus_path=str(personal_path))
	...
	class FakeAgent:
		...
		def run_sync(self, user_prompt, deps=None, **kwargs):
			...
			return FakeRunResult(lpt.LearningProfileResult(success=True, profile=state['profile']))

	fake_agent = FakeAgent()

	monkeypatch.setattr(lpt, "get_user_by_id", lambda user_id: user if user_id == 7 else None)
	monkeypatch.setattr(lpt, "list_user_syllabuses", lambda user_id: [user_syllabus] if user_id == 7 else [])
	monkeypatch.setattr(lpt, "get_syllabus_by_id", lambda syllabus_id: syllabus if syllabus_id == 19 else None)
	...
	profile = lpt.build_learning_profile(
		user_id=7,
		syllabus_id=19,
		dialogue_text="我最近在学 Python，函数参数总是搞不懂，想两周内掌握循环和函数。",
	...
	assert profile["knowledge_mastery"]["by_knowledge_point"]["函数参数"] < profile["knowledge_mastery"]["by_knowledge_point"]["循环嵌套"]
...
def test_build_learning_profile_can_call_context_tools_before_feature_tools(monkeypatch, repo_json_factory):
	personal_path = repo_json_factory(
		"schedule/student_alt",
		{
			"period": [
	...
	user_syllabus = SimpleNamespace(syllabus_id=20, personal_syllabus_path=str(personal_path))
	...
	monkeypatch.setattr(lpt, "list_user_syllabuses", lambda user_id: [user_syllabus] if user_id == 8 else [])
	monkeypatch.setattr(lpt, "get_syllabus_by_id", lambda syllabus_id: syllabus if syllabus_id == 20 else None)
	monkeypatch.setattr(lpt, "get_learning_profile_agent", lambda: fake_agent)
	monkeypatch.setattr(lpt, "_collect_history_entries", lambda *args, **kwargs: history_entries)
	monkeypatch.setattr(lpt, "_load_personal_syllabus", lambda *args, **kwargs: [(20, {"period": [{"week_index": 1, "content": "Python 变量与循环", "enhanced_content": "循环结构与变量作用域", "competance": "weak", "competance_progress": -1}]}, {})])
	monkeypatch.setattr(lpt, "time", lambda: 1760000000)
